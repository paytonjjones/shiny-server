...
server <- function(input, output) {
  output$onesamplet <- renderUI({
    if((input$IV == "0 IVs" | input$IV == "") & (input$DV == "1 interval DV" | input$DV == "") & input$normal == TRUE)  
    { actionLink("onesamplet_link", "One-sample t test") }
  })
  output$onesamplem <- renderUI({
    if((input$IV == "0 IVs" | input$IV == "") & (input$DV == "1 interval DV" & input$normal == FALSE | input$DV == "1 ordinal DV" | input$DV == ""))  
    { actionLink("onesamplem_link", "One-sample median")}
  })
  output$binom <- renderUI({
    if((input$IV == "0 IVs" | input$IV == "") & (input$DV == "1 categorical DV (2 categories)" | input$DV == ""))  
    { actionLink("binom_link","Binomial test") }
  })
  output$chisqgof <- renderUI({
    if((input$IV == "0 IVs" | input$IV == "") & (input$DV == "1 categorical DV (2 categories)" | input$DV == "1 categorical DV (3+ categories)" | input$DV == ""))  
    { actionLink("chisqgof_link", "Chi-square goodness-of-fit test") }
  })
  output$indept <- renderUI({
    if((input$IV == "1 IV with 2 levels (independent groups)" | input$IV == "") & (input$DV == "1 interval DV" |input$DV == "") & input$normal == TRUE)  
    { actionLink("indept_link", "Independent samples t-test") }
  })
  output$wmw <- renderUI({
    if((input$IV == "1 IV with 2 levels (independent groups)" | input$IV == "") & (input$DV == "1 interval DV" & input$normal == FALSE | input$DV == "1 ordinal DV" | input$DV == ""))  
    { actionLink("wmw_link","Wilcoxon-Mann Whitney test") }
  })
  output$chisq <- renderUI({
    if((input$IV == "1 IV with 2 levels (independent groups)" | input$IV == "1 IV with 3+ levels (independent groups)" | input$IV == "") & (input$DV == "1 categorical DV (2 categories)" | input$DV == "1 categorical DV (3+ categories)" | input$DV == ""))  
    { actionLink("chisq_link", "Chi-squared test") }
  })
  output$fisher <- renderUI({
    if((input$IV == "1 IV with 2 levels (independent groups)" | input$IV == "") & (input$DV == "1 categorical DV (2 categories)" | input$DV == "1 categorical DV (3+ categories)" | input$DV == ""))  
    { actionLink("fisher_link", "Fisher's exact test") }
  })
  output$anova <- renderUI({
    if((input$IV == "1 IV with 3+ levels (independent groups)" | input$IV == "") & (input$DV == "1 interval DV" | input$DV == "") & input$normal == TRUE)  
    { actionLink("anova_link", "One-way ANOVA") }
  })
  output$kw <- renderUI({
    if((input$IV == "1 IV with 3+ levels (independent groups)" | input$IV == "") & (input$DV == "1 interval DV" & input$normal == FALSE| input$DV == "1 ordinal DV" | input$DV == ""))  
    { actionLink("kw_link", "Kruskal Wallis test") }
  })
  output$pairedt <- renderUI({
    if((input$IV == "1 IV with 2 levels (dependent/matched groups)" | input$IV == "") & (input$DV == "1 interval DV" | input$DV == "") & input$normal == TRUE)  
    { actionLink("pairedt_link","Paired t-test") }
  })
  output$wsrt <- renderUI({
    if((input$IV == "1 IV with 2 levels (dependent/matched groups)" | input$IV == "") & (input$DV == "1 interval DV" & input$normal == FALSE| input$DV == "1 ordinal DV" | input$DV == ""))  
    { actionLink("wsrt_link", "Wilcoxon signed ranks test") }
  })
  output$mcnemar <- renderUI({
    if((input$IV == "1 IV with 2 levels (dependent/matched groups)" | input$IV == "") & (input$DV == "1 categorical DV (2 categories)" | input$DV == "1 categorical DV (3+ categories)" | input$DV == ""))  
    { actionLink("mcnemar_link", "McNemar test") }
  })
  output$rmanova <- renderUI({
    if((input$IV == "1 IV with 3+ levels (dependent/matched groups)" | input$IV == "") & (input$DV == "1 interval DV" | input$DV == "") & input$normal == TRUE)  
    { actionLink("rmanova_link","One-way repeated measures ANOVA") }
  })
  output$friedman <- renderUI({
    if((input$IV == "1 IV with 3+ levels (dependent/matched groups)" | input$IV == "") & (input$DV == "1 interval DV" & input$normal == FALSE | input$DV == "1 ordinal DV" | input$DV == ""))  
    { actionLink("friedman_link","Friedman test") }
  })
  output$rmlog <- renderUI({
    if((input$IV == "1 IV with 3+ levels (dependent/matched groups)" | input$IV == "") & (input$DV == "1 categorical DV (2 categories)" | input$DV == "1 categorical DV (3+ categories)" | input$DV == ""))  
    { actionLink("rmlog_link","Repeated measures logistic regression") }
  })
  output$facanova <- renderUI({
    if((input$IV == "2+ IVs (all categorical with independent groups)" | input$IV == "") & (input$DV == "1 interval DV" | input$DV == "") & input$normal == TRUE)  
    { actionLink("facanova_link","Factorial ANOVA") }
  })
  output$ordlog <- renderUI({
    if((input$IV == "2+ IVs (all categorical with independent groups)" | input$IV == "2+ IVs (interval or mixed types)" | input$IV == "") & (input$DV == "1 interval DV" & input$normal == FALSE | input$DV == "1 ordinal DV" | input$DV == ""))
    { actionLink("ordlog_link","Ordered logistic regression") }
  })
  output$faclog <- renderUI({
    if((input$IV == "2+ IVs (all categorical with independent groups)" | input$IV == "") & (input$DV == "1 categorical DV (2 categories)" | input$DV == "1 categorical DV (3+ categories)" | input$DV == ""))  
    { actionLink("faclog_link","Factorial logistic regression") }
  })
  output$corr <- renderUI({
    if((input$IV == "1 interval IV" | input$IV == "") & (input$DV == "1 interval DV" | input$DV == "") & input$normal == TRUE)  
  ...
  output$npc <- renderUI({
    if((input$IV == "1 interval IV" | input$IV == "") & (input$DV == "1 interval DV" & input$normal == FALSE | input$DV == "1 ordinal DV" | input$DV == ""))
    { actionLink("npc_link","Non-parametric correlation") }
  })
  output$log <- renderUI({
    if((input$IV == "1 interval IV" | input$IV == "") & (input$DV == "1 categorical DV (2 categories)" | input$DV == "1 categorical DV (3+ categories)" | input$DV == ""))  
    { actionLink("log_link","Logistic regression") }
  })
  output$multreg <- renderUI({
    if((input$IV == "2+ IVs (interval or mixed types)" | input$IV == "") & (input$DV == "1 interval DV" | input$DV == "") & input$normal == TRUE)
  ...
  output$multlog <- renderUI({
    if((input$IV == "2+ IVs (interval or mixed types)" | input$IV == "") & (input$DV == "1 categorical DV (2 categories)" | input$DV == "1 categorical DV (3+ categories)" | input$DV == ""))  
  ...
  output$manova <- renderUI({
    if((input$IV == "1 IV with 2 levels (independent groups)" | input$IV == "1 IV with 3+ levels (independent groups)" | input$IV == "") & (input$DV == "2+ interval DVs" | input$DV == "") & input$normal== TRUE)  
    { actionLink("manova_link","One-way MANOVA") }
  })
  output$manova2 <- renderUI({
    if((input$IV == "1 IV with 2 levels (dependent/matched groups)" | input$IV == "1 IV with 3+ levels (dependent/matched groups)" | input$IV == "") & (input$DV == "2+ interval DVs" | input$DV == "") & input$normal== TRUE)  
    { actionLink("manova2_link","Repeated-measures MANOVA") }
  })
  output$mmreg <- renderUI({
    if((input$IV == "2+ IVs (interval or mixed types)" | input$IV == "2+ IVs (all categorical with independent groups)" | input$IV == "") & (input$DV == "2+ interval DVs" | input$DV == "") & input$normal== TRUE)  
    { actionLink("mmreg_link","Multivariate multiple linear regression") }
  })
  output$factora <- renderUI({
    if((input$IV == "0 IVs" | input$IV == "") & (input$DV == "2+ interval DVs" | input$DV == "") & input$normal== TRUE)  
    { actionLink("factora_link","Factor analysis") }
  })
  
  explain <- reactiveValues(data = NULL)
  observeEvent(input$onesamplet_link, {
    explain$wiki <- HTML("<h2 style=\"display: inline;\">Student's t-test</h2>
                         <body><div> from Wikipedia, the free encyclopedia </div></body>
                         <p><div>A t-test is any statistical hypothesis test in which the test statistic follows a Student's t-distribution under the null hypothesis. It can be used to determine if two sets of data are significantly different from each other.</div></p>
                         <p><div> A t-test is most commonly applied when the test statistic would follow a normal distribution if the value of a scaling term in the test statistic were known. When the scaling term is unknown and is replaced by an estimate based on the data, the test statistics (under certain conditions) follow a Student's t distribution.</div></p>
                         <h3><div>One-sample t-test in R  </div></h3>
                         <code> t.test(vector, mu = 0)</code>
                         ")})
  observeEvent(input$onesamplem_link, {
    explain$wiki <- HTML("<h2 style=\"display: inline;\">One-sample median</h2>
  ...
  observeEvent(input$binom_link, {
    explain$wiki <- HTML("<h2 style=\"display: inline;\">Binomial test</h2>
                         <body><div> from Wikipedia, the free encyclopedia </div></body>
  ...
  observeEvent(input$chisqgof_link, {
    explain$wiki <- HTML("<h2 style=\"display: inline;\">Pearson's chi-squared test</h2>
                         <body><div> from Wikipedia, the free encyclopedia </div></body>
  ...
  observeEvent(input$indept_link, {
    explain$wiki <- HTML("<h2 style=\"display: inline;\">Independent (unpaired) samples t-test</h2>
                         <body><div> from Wikipedia, the free encyclopedia </div></body>
  ...
  observeEvent(input$wmw_link, {
    explain$wiki <- HTML("<h2 style=\"display: inline;\">Mann-Whitney U Test</h2>
                         <body><div> from Wikipedia, the free encyclopedia </div></body>
  ...
  observeEvent(input$chisq_link, {
    explain$wiki <- HTML("<h2 style=\"display: inline;\">Chi-squared test</h2>
                         <body><div> from Wikipedia, the free encyclopedia </div></body>
  ...
  observeEvent(input$fisher_link, {
    explain$wiki <- HTML("<h2 style=\"display: inline;\">Fisher's exact test</h2>
                         <body><div> from Wikipedia, the free encyclopedia </div></body>
  ...
  observeEvent(input$anova_link, {
    explain$wiki <- HTML("<h2 style=\"display: inline;\">One-way analysis of variance</h2>
                         <body><div> from Wikipedia, the free encyclopedia </div></body>
  ...
  observeEvent(input$kw_link, {
    explain$wiki <- HTML("<h2 style=\"display: inline;\">Kruskal-Wallis one-way ANOVA</h2>
                         <body><div> from Wikipedia, the free encyclopedia </div></body>
  ...
  observeEvent(input$pairedt_link, {
    explain$wiki <- HTML("<h2 style=\"display: inline;\">Paired samples t-test</h2>
                         <body><div> from Wikipedia, the free encyclopedia </div></body>
  ...
  observeEvent(input$wsrt_link, {
    explain$wiki <- HTML("<h2 style=\"display: inline;\">Wilcoxon signed-rank test</h2>
                         <body><div> from Wikipedia, the free encyclopedia </div></body>
  ...
  observeEvent(input$mcnemar_link, {
    explain$wiki <- HTML("<h2 style=\"display: inline;\">McNemar's test</h2>
                         <body><div> from Wikipedia, the free encyclopedia </div></body>
  ...
  observeEvent(input$rmanova_link, {
    explain$wiki <- HTML("<h2 style=\"display: inline;\">Repeated measures ANOVA</h2>
                         <body><div> from Wikipedia, the free encyclopedia </div></body>
  ...
  observeEvent(input$friedman_link, {
    explain$wiki <- HTML("<h2 style=\"display: inline;\">Friedman test</h2>
                         <body><div> from Wikipedia, the free encyclopedia </div></body>
  ...
  observeEvent(input$rmlog_link, {
    explain$wiki <- HTML("<h2 style=\"display: inline;\">Repeated measures logistic regression</h2>
                         <body><div> from Wikipedia, the free encyclopedia </div></body>
  ...
  observeEvent(input$facanova_link, {
    explain$wiki <- HTML("<h2 style=\"display: inline;\">Two-way analysis of variance</h2>
                         <body><div> from Wikipedia, the free encyclopedia </div></body>
  ...
  observeEvent(input$ordlog_link, {
    explain$wiki <- HTML("<h2 style=\"display: inline;\">Ordered logit</h2>
                         <body><div> from Wikipedia, the free encyclopedia </div></body>
  ...
  observeEvent(input$faclog_link, {
    explain$wiki <- HTML("<h2 style=\"display: inline;\">Factorial logistic regression</h2>
                         <body><div> from Wikipedia, the free encyclopedia </div></body>
                         <p><div></div>In statistics, logistic regression, or logit regression, or logit model is a regression model where the dependent variable (DV) is categorical. This article covers the case of a binary dependent variable; that is, where it can take only two values, \"0\" and \"1\", which represent outcomes such as pass/fail, win/lose, alive/dead or healthy/sick. Cases where the dependent variable has more than two outcome categories may be analysed in multinomial logistic regression, or, if the multiple categories are ordered, in ordinal logistic regression. In the terminology of economics, logistic regression is an example of a qualitative response/discrete choice model.</p>
  ...
  observeEvent(input$corr_link, {
    explain$wiki <- HTML("<h2 style=\"display: inline;\">Pearson correlation coefficient</h2>
                         <body><div> from Wikipedia, the free encyclopedia </div></body>
  ...
  observeEvent(input$reg_link, {
    explain$wiki <- HTML("<h2 style=\"display: inline;\">Regression analysis</h2>
                         <body><div> from Wikipedia, the free encyclopedia </div></body>
  ...
  observeEvent(input$npc_link, {
    explain$wiki <- HTML("<h2 style=\"display: inline;\">Spearman's rank correlation coefficient</h2>
                         <body><div> from Wikipedia, the free encyclopedia </div></body>
  ...
  observeEvent(input$log_link, {
    explain$wiki <- HTML("<h2 style=\"display: inline;\">Logistic regression</h2>
                         <body><div> from Wikipedia, the free encyclopedia </div></body>
  ...
  observeEvent(input$multreg_link, {
    explain$wiki <- HTML("<h2 style=\"display: inline;\">Simple and multiple regression</h2>
                         <body><div> from Wikipedia, the free encyclopedia </div></body>
  ...
  observeEvent(input$ancova_link, {
    explain$wiki <- HTML("<h2 style=\"display: inline;\">Analysis of covariance</h2>
                         <body><div> from Wikipedia, the free encyclopedia </div></body>
  ...
  observeEvent(input$multlog_link, {
    explain$wiki <- HTML("<h2 style=\"display: inline;\">Multiple logistic regression</h2>
  ...
  observeEvent(input$disc_link, {
    explain$wiki <- HTML("<h2 style=\"display: inline;\">Linear discriminant analysis</h2>
                         <body><div> from Wikipedia, the free encyclopedia </div></body>
  ...
  observeEvent(input$manova_link, {
    explain$wiki <- HTML("<h2 style=\"display: inline;\">Multivariate analysis of variance</h2>
                         <body><div> from Wikipedia, the free encyclopedia </div></body>
  ...
  observeEvent(input$manova2_link, {
    explain$wiki <- HTML("<h2 style=\"display: inline;\">Repeated measures MANOVA</h2>
                         <body><div> from Wikipedia, the free encyclopedia </div></body>
                         <p><div>In statistics, multivariate analysis of variance (MANOVA) is a procedure for comparing multivariate sample means. As a multivariate procedure, it is used when there are two or more dependent variables, and is typically followed by significance tests involving individual dependent variables separately. It helps to answer</div></p>
                         <p><div>1. Do changes in the independent variable(s) have significant effects on the dependent variables?</div></p>
                         <p><div>2. What are the relationships among the dependent variables?</div></p>
  ...
  observeEvent(input$mmreg_link, {
    explain$wiki <- HTML("<h2 style=\"display: inline;\">Multivariate multiple regression</h2>
  ...
  observeEvent(input$factora_link, {
    explain$wiki <- HTML("<h2 style=\"display: inline;\">Factor analysis</h2>
                         <body><div> from Wikipedia, the free encyclopedia </div></body>
